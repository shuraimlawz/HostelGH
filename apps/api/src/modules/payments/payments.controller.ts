...
export class PaymentsController {
    ...
    submitProof(@Req() req: any, @Param("bookingId") bookingId: string, @Body() dto: SubmitProofDto) {
        return this.payments.submitOfflineProof(req.user.userId, bookingId, dto.proofUrl, dto.notes);
    }

    @Post("offline/verify/:paymentId")
    @Roles(UserRole.OWNER, UserRole.ADMIN)
    verifyOffline(@Req() req: any, @Param("paymentId") paymentId: string, @Body("status") status: "SUCCESS" | "FAILED") {
        return this.payments.verifyOfflinePayment(req.user.userId, paymentId, status);
    }
}
