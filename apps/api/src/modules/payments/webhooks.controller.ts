...
export class WebhooksController {
  ...
  @ApiOperation({
    summary: "Handle Paystack webhook events (signature verified)",
  })
  async handlePaystack(
    ...
    if (!rawBody)
      throw new BadRequestException(
        "Raw body not available for signature check",
      );
    ...
    if (
      ...
      !timingSafeEqual(sigBuffer, compBuffer)
    ) {
    ...
    this.logger.log(
      `Paystack Webhook Received: ${event?.event} [Ref: ${event?.data?.reference}]`,
    );
