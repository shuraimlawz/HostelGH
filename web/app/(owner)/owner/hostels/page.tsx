        ...
        return (
            <div className="flex h-[60vh] items-center justify-center">
                <div className="flex flex-col items-center gap-4 text-center">
                    ...
                    <p className="text-sm font-medium text-gray-400">Syncing portfolio assets...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="max-w-[1400px] mx-auto space-y-10 pb-20 pt-4 px-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                <div className="space-y-2">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Property Network</span>
                    </div>
                    <h1 className="text-3xl md:text-4xl font-bold text-gray-900 tracking-tight">Portfolio</h1>
                    <p className="text-gray-500 text-sm max-w-md">Enterprise-grade control over your property fleet and operational network.</p>
                </div>
            ...
            <div className="flex items-center justify-between border-b border-gray-100 pb-4">
                ...
                <div className="text-[10px] font-bold uppercase tracking-widest text-gray-400">
                    Active Listings: <span className="text-gray-900">{hostels?.length || 0}</span>
                </div>
            </div>
            ...
            {filteredHostels?.length === 0 ? (
                <div className="bg-white border border-gray-100 border-dashed rounded-2xl p-20 text-center space-y-6">
                    ...
                    <div className="space-y-1">
                        ...
                        <p className="text-sm text-gray-500 max-w-sm mx-auto">Either you haven't listed any hostels yet or no assets match your search.</p>
                    </div>
                    <Link
                        href="/owner/hostels/new"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {filteredHostels?.map((hostel: any) => (
                        <div key={hostel.id} className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:border-blue-200 transition-all duration-300 flex flex-col shadow-sm hover:shadow-xl hover:shadow-blue-500/5">
                            {/* Visual Layer */}
                            <div className="aspect-[16/10] bg-gray-50 relative overflow-hidden">
                                ...
                                <div className="absolute inset-x-3 top-3 flex justify-between items-start">
                                    ...
                                    <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                            <button className="w-8 h-8 flex items-center justify-center bg-white/80 hover:bg-white backdrop-blur-md rounded-lg transition-all text-gray-900 shadow-sm border border-white/40">
                                                <MoreVertical size={16} />
                                            </button>
                                        </DropdownMenuTrigger>
                                        <DropdownMenuContent align="end" className="w-48 rounded-xl p-2 shadow-xl border border-gray-100 bg-white">
                                            <DropdownMenuItem asChild className="rounded-lg p-2.5 font-bold text-xs cursor-pointer hover:bg-gray-50 flex items-center gap-3">
                                            ...
                                            <DropdownMenuItem asChild className="rounded-lg p-2.5 font-bold text-xs cursor-pointer hover:bg-gray-50 flex items-center gap-3">
                                                <Link href={`/owner/hostels/${hostel.id}/edit`}>
                                                    <Settings2 size={14} className="text-gray-500" /> Operational Config
                                                </Link>
                                            </DropdownMenuItem>
                                ...
                                <div className="absolute inset-x-3 bottom-3 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                                    <div className="bg-white/95 backdrop-blur-md rounded-xl p-3 border border-white shadow-lg flex items-center justify-between">
                                        <div className="flex items-center gap-2 overflow-hidden">
                                            <MapPin size={12} className="text-blue-600 shrink-0" />
                                            <span className="text-[10px] font-bold text-gray-900 truncate">{hostel.city}</span>
                                        </div>
                            ...
                            <div className="p-6 flex-1 flex flex-col space-y-6">
                                <div className="space-y-1">
                                    <h3 className="text-lg font-bold text-gray-900 tracking-tight truncate group-hover:text-blue-600 transition-colors">
                                        {hostel.name}
                                    </h3>
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">UID: {hostel.id.slice(-6).toUpperCase()}</p>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="bg-gray-50 rounded-xl p-3 border border-gray-100 space-y-0.5">
                                        ...
                                        <p className="text-sm font-bold text-gray-900">{hostel._count.rooms}</p>
                                    </div>
                                    <div className="bg-gray-50 rounded-xl p-3 border border-gray-100 space-y-0.5">
                                        <div className="flex items-center gap-1.5 text-gray-400">
                                        ...
                                        <p className="text-sm font-bold text-gray-900">{hostel._count.bookings}</p>
                                    </div>
                                </div>
