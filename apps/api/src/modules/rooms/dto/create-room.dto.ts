import { IsString, IsNotEmpty, IsOptional, IsInt, Min, IsBoolean, IsEnum, IsArray } from "class-validator";
...
export class CreateRoomDto {
    ...
    @IsString() @IsNotEmpty() roomConfiguration!: string;

    @ApiProperty({ example: "MIXED", enum: ["MALE", "FEMALE", "MIXED"] })
    @IsEnum(["MALE", "FEMALE", "MIXED"]) gender!: string;

    @ApiProperty({ example: 10, description: "Total available slots" })
    @IsInt() @Min(1) totalSlots!: number;

    @ApiProperty({ example: 10, description: "Currently available slots" })
    @IsInt() @Min(0) availableSlots!: number;

    @ApiProperty({ example: false, default: false })
    @IsOptional() @IsBoolean() hasAC?: boolean = false;
