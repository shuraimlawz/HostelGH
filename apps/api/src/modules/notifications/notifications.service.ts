...
export class NotificationsService {
  ...
  async sendBookingRequestSms(
    to: string,
    payload: { tenantName: string; hostelName: string },
  ) {
  ...
  async sendPaymentConfirmedSms(
    to: string,
    payload: { hostelName: string; amount: string },
  ) {
    const message = `Payment of ${payload.amount} for ${payload.hostelName} confirmed. Your slot is now locked. Welcome home!`;
    return this.sms.sendSms(to, message);
  }

  async sendBroadcastEmail(to: string, payload: BroadcastPayload) {
    return this.email.send({
      to,
